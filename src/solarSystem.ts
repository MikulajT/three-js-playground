...
function main() : void {
  ...
  const renderer = new THREE.WebGLRenderer( { antialias: true, canvas } );
	...
	// Create a GUI for controlling helper visibility
	const gui = new GUI();
  ...
  objects.push( earthMesh );
	...
	// Helper function to create an AxisGridHelper and add it to the GUI
	function makeAxisGrid(node: THREE.Object3D, label: string, units?: number) {
		const helper = new AxisGridHelper(node, units);
		gui.add(helper, 'visible').name(label);
	}
	
	// Create axis/grid helpers for each major object
